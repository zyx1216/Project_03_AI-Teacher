...
_TOC_DOTTED_RE = re.compile(
    r"^\s*(?P<title>.+?)\s*[\.．。·•…]{2,}\s*(?P<page>[0-9]+)\s*$")
# 无点线时，只接受明确教材编号标题，避免普通正文误入目录
_TOC_NUMBERED_RE = re.compile(
    r"^\s*(?P<title>第\s*[0-9一二三四五六七八九十百零〇]+\s*[章节单元课]"
    r"(?:\s*[^\s0-9。！？；，、][^\n]{0,40})?)\s+(?P<page>[0-9]+)\s*$")
_TOC_FIXED_RE = re.compile(
    r"^\s*(?P<title>(?:整理与复习|总复习|综合实践|练一练)"
    r"(?:\s+[^\n]{1,40})?)\s+(?P<page>[0-9]+)\s*$")
_TOC_NUMERIC_HEADING_RE = re.compile(
    r"^\s*(?P<title>[0-9]{1,2}(?:\.[0-9]{1,2})+\s+[^\n]{1,60}?)"
    r"\s+(?P<page>[0-9]+)\s*$")
...
def extract_toc_by_regex(text: str) -> list[dict]:
    ...
    for line in str(text or "").splitlines():
        match = (_TOC_DOTTED_RE.match(line) or _TOC_NUMBERED_RE.match(line)
                 or _TOC_FIXED_RE.match(line) or _TOC_NUMERIC_HEADING_RE.match(line))
        if match is None:
            continue
...
def _looks_like_toc_page(text: str) -> bool:
    """判断页面是否仍为目录页。"""
    lines = str(text or "").splitlines()
    dotted = sum(1 for line in lines if _TOC_DOTTED_RE.match(line))
    return dotted >= 2 or "目录" in text


def detect_page_offset(file_bytes: bytes, toc_pages: list[dict]) -> int:
    """检测偏移量：PDF页码 = 印刷页码 + offset；失败返回 0。"""
    import pymupdf
    ...
    first = toc_pages[0]
    try:
        printed_page = int(first.get("page"))
    except (TypeError, ValueError):
        return 0
    compact_title = _compact_for_match(first.get("title"))
    if printed_page <= 0 or not compact_title:
        return 0

    with pymupdf.open(stream=io.BytesIO(bytes(file_bytes)), filetype="pdf") as doc:
        page_texts = [page.get_text("text") for page in doc]
    total = len(page_texts)
    toc_index = next((i for i, text in enumerate(page_texts[:10]) if "目录" in text), -1)
    start_index = toc_index + 1 if toc_index >= 0 else 0
    end_index = min(total, start_index + 20)

    for index in range(start_index, end_index):
        if _looks_like_toc_page(page_texts[index]):
            continue
        if compact_title in _compact_for_match(page_texts[index]):
            offset = index + 1 - printed_page
            if offset >= 0:
                return offset

    if toc_index >= 0:
        estimate = toc_index + 1 - 5
        if 0 <= estimate <= 10:
            return estimate
    return 0


def apply_page_offset(chapters: list[dict], offset: int, page_count: int) -> list[dict]:
    """把目录印刷页码转换成 PDF 页码，并裁剪到 PDF 页数范围。"""
    try:
        offset = int(offset)
        total = max(1, int(page_count))
    except (TypeError, ValueError) as exc:
        raise ValueError("页码偏移量或 PDF 页数不是数字。") from exc
    ...
    for item in chapters or []:
        ...
        try:
            ...
            continue
        page = min(max(page + offset, 1), total)
        level = min(max(level, 1), 3)
        key = (title, level, page)
        if key not in seen:
            seen.add(key)
...
def detect_pdf_chapters(file_bytes: bytes, chat_func,
                       return_details: bool = False):
    """识别 PDF 章节：目录正则优先，正则不足时使用 AI。

    return_details=False 返回章节列表，兼容旧调用；True 时同时返回偏移和来源。
    """
    import pymupdf
    ...
    printed_chapters = extract_toc_by_regex(source)
    if len(printed_chapters) >= 3:
        toc_source = "regex"
    else:
        system_prompt = (
        ...
        printed_chapters = parse_ai_chapters(raw, page_count)
        toc_source = "ai"

    offset = detect_page_offset(file_bytes, printed_chapters)
    chapters = apply_page_offset(printed_chapters, offset, page_count)
    if not chapters:
        raise ValueError("没有识别到有效章节。")
    if return_details:
        return {
            "chapters": chapters,
            "printed_chapters": printed_chapters,
            "offset": offset,
            "source": toc_source,
        }
    return chapters
