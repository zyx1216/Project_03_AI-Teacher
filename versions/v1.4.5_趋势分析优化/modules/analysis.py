...
from utils import academic_time, analysis_settings
...
def tab_scores():
    ...
    with st.expander("📊 历次考试成绩", expanded=False):
...
def _score_import_panel():
    ...
    st.success(f"识别到姓名列「{detected['name_col']}」，科目："
               + "、".join(subject for _, subject in detected["subjects"]))
...
def _render_subject_view(data, rank_order):
    ...
    st.divider()
    _render_band_chart(data["stats"]["bands"], subject)

    st.divider()
    st.markdown(f"**{subject}学生排名图**")
    _render_rank_bar_chart(
        data["rows"], subject, f"{subject}单科排名（{rank_order}）",
        subject, high_to_low)
...
def _trend_scope_options(exams) -> list[str]:
    """显示范围选项：最近次数 + 按日期推导出的学期、学年。"""
    options = ["最近 3 次", "最近 5 次", "最近 10 次", "全部"]
    semesters = sorted(
        {academic_time.semester_name(e.exam_date) for e in exams
         if e.exam_date is not None},
        key=lambda name: min(
            academic_time.semester_key(e.exam_date)
            for e in exams
            if e.exam_date is not None
            and academic_time.semester_name(e.exam_date) == name))
    years = sorted({
    ...
    options.extend(f"学期：{name}" for name in semesters)
    options.extend(f"学年：{name}" for name in years)
    return options


def _trend_exam_ids_by_scope(exams, scope: str) -> tuple[list[int], int | None]:
    """按显示范围筛选考试；最近次数返回全部考试 ID 和截断次数。"""
    if scope in ("最近 3 次", "最近 5 次", "最近 10 次"):
        return [e.id for e in exams], int(scope.split()[1])
    if scope == "全部":
        return [e.id for e in exams], None
    if scope.startswith("学期："):
        wanted = scope.replace("学期：", "", 1)
        ids = [e.id for e in exams
               if e.exam_date is not None
               and academic_time.semester_name(e.exam_date) == wanted]
        return ids, None
    if scope.startswith("学年："):
        wanted = scope.replace("学年：", "", 1)
        ids = [e.id for e in exams
               if e.exam_date is not None
               and academic_time.academic_year_name(e.exam_date) == wanted]
        return ids, None
    return [e.id for e in exams], None


def tab_trends():
    """班级趋势和个人趋势；支持最近次数、学期、学年三类范围。"""
    st.subheader("趋势分析")
    ...
    c1, c2, c3 = st.columns(3)
    scope = c1.selectbox(
        "显示范围", _trend_scope_options(exams), index=1, key="trend_scope")
    class_filter = c2.selectbox("班级", ["全部"] + classes, key="trend_class")
    trend_subject = c3.selectbox(
        "科目", ["全部科目"] + trend_subjects, index=0, key="trend_subject")

    selected_exam_ids, limit = _trend_exam_ids_by_scope(exams, scope)
    if not selected_exam_ids:
...
def _class_trend_chart(class_filter, subject=None, exam_ids=None, limit=None):
    ...
    labels = trend["labels"][start:]
    if not labels:
        st.caption("所选范围内没有数据。")
        return

    def show_figure(series_keys: list[str], title: str, total_chart: bool = False):
        fig = go.Figure()
        for item_subject in series_keys:
            values = trend["series"][item_subject][start:]
            fig.add_trace(go.Scatter(
                x=labels, y=values, mode="lines+markers+text" if total_chart else "lines+markers",
                text=values if total_chart else None,
                textposition="top center", name="总分" if total_chart else item_subject,
                connectgaps=False,
                line=dict(width=4, color="#1F4E79") if total_chart else None))
        fig.update_layout(
            height=420, margin=dict(l=10, r=10, t=40, b=10),
            title=title, yaxis_title="平均分", hovermode="x unified",
            legend=dict(orientation="h", yanchor="bottom", y=1.02))
        st.plotly_chart(fig, width="stretch")

    if subject is not None:
        show_figure([subject], f"{subject}平均分趋势")
    else:
        show_figure(trend["subjects"], "各科平均分趋势")
        show_figure(["total"], "总分平均分趋势", total_chart=True)


def _student_trend_panel(class_filter, subject=None, exam_ids=None, limit=None):
    """学生个人趋势；单科成绩和总分固定拆成两张图。"""
    with SessionLocal() as session:
        students = ss.list_students(
            session,
            class_name=None if class_filter == "全部" else class_filter)
    ...
    x_labels = [labels.get(h["exam_id"], h["exam_name"]) for h in history]
    ...
    def add_subject_trace(fig, item_subject, is_bar: bool):
        values = [h.get(item_subject) for h in history]
        if is_bar:
            fig.add_trace(go.Bar(x=x_labels, y=values, name=item_subject))
        else:
            fig.add_trace(go.Scatter(
                x=x_labels, y=values, mode="lines+markers",
                name=item_subject, connectgaps=False))

    subject_fig = go.Figure()
    total_fig = go.Figure()
    is_bar = chart_type == "柱状图"
    for item_subject in shown_subjects:
        add_subject_trace(subject_fig, item_subject, is_bar)
    total_values = [h.get("total") for h in history]
    if is_bar:
        total_fig.add_trace(go.Bar(
            x=x_labels, y=total_values, name="总分",
        ...
        total_fig.add_trace(go.Scatter(
            x=x_labels, y=total_values, mode="lines+markers+text",
            text=total_values, textposition="top center", name="总分",
            line=dict(width=5, color="#1F4E79"), connectgaps=False))

    layout = dict(
        height=400, margin=dict(l=10, r=10, t=40, b=10),
        yaxis_title="分数", hovermode="x unified",
        legend=dict(orientation="h", yanchor="bottom", y=1.02))
    subject_fig.update_layout(title="各科成绩趋势", **layout)
    total_fig.update_layout(title="总分趋势", **layout)
