...
def _isolated_app_code(db_file: Path, feature_file: Path | None = None,
                       ...
                       thresholds_file: Path | None = None) -> str:
...
def test_trend_range_default_and_chart_switch_isolated(tmp_path):
    ...
    assert scope.options[:4] == ["最近 3 次", "最近 5 次", "最近 10 次", "全部"]
    assert any(x.startswith("学期：") for x in scope.options)
    assert any(x.startswith("学年：") for x in scope.options)
    assert scope.value == "最近 5 次"
    scope.set_value("学期：2026春").run()
    assert not at.exception, [str(e) for e in at.exception]
...
def test_v140_score_template_button_and_empty_states(tmp_path):
    ...
    assert "还没有资料" in lesson_info
    assert "还没有教材资料" in lesson_info
...
def test_v142_settings_class_management_and_preset_class(tmp_path):
    """设置页有班级管理；JSON 预设空班级会出现在学情班级下拉中。"""
    db_file = tmp_path / "v142_settings.db"
    feature_file = tmp_path / "feature.json"
    ...
    assert "班级管理" in subheaders
    assert any(x.label == "添加班级" for x in at.text_input)
    assert any("批量调整学生班级" in str(x.value) for x in at.markdown)
...
def test_v145_trend_scope_unique_labels_and_two_charts_isolated(tmp_path):
    """趋势无预测入口；范围含学期/学年，X 轴使用带学期的唯一标签。"""
    ...
    scope = next(x for x in at.selectbox if x.key == "trend_scope")
    assert scope.options[:4] == ["最近 3 次", "最近 5 次", "最近 10 次", "全部"]
    assert "学期：2026春" in scope.options
    assert "学年：2025-2026学年" in scope.options
    # 全部科目下两张班级图 + 两张个人图均在同一轮渲染；AppTest 不暴露
    # st.plotly_chart 元素，至少验证无异常且后续切学期/学年仍正常。
    scope.set_value("学年：2025-2026学年").run()
    assert not at.exception, [str(e) for e in at.exception]
    scope.set_value("全部").run()
    assert not at.exception, [str(e) for e in at.exception]
...
def test_v145_settings_thresholds_saved_isolated(tmp_path):
    """设置页可保存自定义及格线/优秀线，且写入临时配置文件。"""
    db_file = tmp_path / "v145_thresholds.db"
    ...
    pass_box = next(x for x in at.number_input if x.label == "及格线百分比")
    excellent_box = next(x for x in at.number_input if x.label == "优秀线百分比")
    pass_box.set_value(70).run()
    excellent_box.set_value(90).run()
    next(b for b in at.button if b.label == "💾 保存考试分析线").click().run()
    assert not at.exception, [str(e) for e in at.exception]
