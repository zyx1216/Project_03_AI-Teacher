# AI 教师工作台 · AGENTS.md（v1.2 支持 9 学科切换）

> 本文件只在本项目目录内生效；全局规则见 `~/.codex/AGENTS.md`，此处不复制。

## 项目概览
- 目标：单人中小学教师本地使用的 AI 教学工作台（备课 / 出题 / 作业 / 学情分析），v1.2 起支持语文、数学、英语、物理、化学、生物、政治、历史、地理 9 学科全局切换。
...
## 架构约定
- 4 个侧边栏页面：备课(modules/lesson_plan.py，5 tab，资料支持 PDF/Word/文本/网页链接)、作业(homework.py，4 tab)、学情(analysis.py，7 tab)、设置(settings.py，含数据管理区和当前学科切换)。每页内部用 st.tabs，每 tab 一个函数，show() 只路由。新建作业、新建考试都用 @st.dialog 弹窗。
...
## 测试与验证
- tests/：test_stats.py（统计口径）、test_excel_import.py（内存库跑识别→入库→分析全链路）、test_app_smoke.py（AppTest 4 主页 + 学情 7 tab，空库/有数据用临时 SQLite+runpy 隔离引擎；学科 UI 用临时 app_config.json）。
