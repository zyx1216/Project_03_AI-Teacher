...
def test_settings_current_subject_switch_updates_config_and_title():
    """设置页切换学科：写临时 app_config.json，rerun 后侧边栏标题和图标跟随变化。"""
    import json
    ...
    at.sidebar.radio[0].set_value("⚙️ 设置").run()
    assert not at.exception, [str(e) for e in at.exception]
    assert any("选择你当前任教的学科" in str(x.value) for x in at.caption)

    box = next(x for x in at.selectbox if x.label == "当前学科")
    box.set_value("物理").run()
    assert not at.exception, [str(e) for e in at.exception]
    assert any("学科已切换" in str(x.value) for x in at.success)

    config_path = None
    from utils import app_config
    config_path = app_config.APP_CONFIG_PATH
    assert json.loads(config_path.read_text(encoding="utf-8")) == {
        "current_subject": "物理"
    }

    at2 = AppTest.from_file(str(APP_FILE), default_timeout=30)
    at2.run()
    assert not at2.exception, [str(e) for e in at2.exception]
    assert any("⚛️ AI教学辅助·物理" in str(x.value) for x in at2.sidebar.title)
...
def test_homework_new_dialog_persists_switch_and_creates_isolated(tmp_path):
    """弹窗内切换类型不闪退；试卷默认 120/90；创建真实写入临时库。"""
...
def test_student_editor_and_trend_controls_isolated(tmp_path):
    ...
    editor = next(x for x in at.dataframe if x.key and x.key.startswith("student_editor_"))
...
def test_lesson_materials_web_link_preview_and_save_isolated(tmp_path, monkeypatch):
    ...
    at = AppTest.from_string(_isolated_app_code(db_file, app_config_file), default_timeout=30)
    at.run()
    assert "⚛️ AI教学辅助·物理" in " ".join(x.value for x in at.sidebar.title)
    at.sidebar.radio[0].set_value("📚 备课").run()
