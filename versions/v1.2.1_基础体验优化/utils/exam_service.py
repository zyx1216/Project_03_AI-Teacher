...
def list_exams(session) -> list[Exam]:
    """所有考试按日期升序返回（趋势分析依赖这个顺序）。"""
    return session.query(Exam).order_by(Exam.exam_date, Exam.id).all()
...
def class_trend(session, class_name: str | None = None) -> dict:
    """
    ...
    for exam in exams:
        ...
        if not rows:
            continue
        subjects = exam_subjects(session, exam.id)
