...
import config
from utils import app_config, llm_client, backup_service
...
def _current_subject_panel():
    """当前任教学科切换：影响应用标题、默认资料归属和导出文件名。"""
    st.subheader("当前学科")
    st.caption("选择你当前任教的学科，应用标题和默认设置会跟着切换。已有的成绩数据不受影响。")
    current_subject = app_config.get_current_subject()
    subject = st.selectbox(
        "当前学科", app_config.ALL_SUBJECTS,
        index=app_config.ALL_SUBJECTS.index(current_subject),
        key="current_subject_select")
    if subject != current_subject:
        app_config.set_current_subject(subject)
        st.success("学科已切换")
...
def _about_panel():
    """关于信息。"""
    st.subheader("关于")
    st.write(f"**项目名称：** {config.get_app_name()}")
    ...
    st.markdown(
        ...
        **四大模块（v1.2.1）**
        ...
        - 设置：当前学科切换、主模型 / 内容模型 / 向量模型配置、数据备份恢复清空
        - 共 13 张数据表；详细操作见项目根目录的 `USAGE.md`。
