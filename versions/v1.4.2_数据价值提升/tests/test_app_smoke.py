...
def _isolated_app_code(db_file: Path, feature_file: Path | None = None,
                       class_file: Path | None = None) -> str:
    """生成在临时数据库、临时功能学科和班级名单上跑 app.py 的脚本。"""
...
def test_v140_score_template_button_and_empty_states(tmp_path):
    ...
    assert "请先在上方“考试管理”里新建一场考试" in analysis_info
...
def test_v142_trend_prediction_controls_isolated(tmp_path):
    """趋势页有预测开关和固定提示；有数据时渲染预测图不抛异常。"""
    db_file = tmp_path / "v142_trend.db"
    _seed_temp_db(db_file)
    at = AppTest.from_string(
        _isolated_app_code(db_file, tmp_path / "feature.json"), default_timeout=30)
    at.run()
    ...
    prediction = next(x for x in at.checkbox if x.key == "trend_show_prediction")
    assert prediction.value is True
    captions = " ".join(str(x.value) for x in at.caption)
    assert "预测仅供参考" in captions
