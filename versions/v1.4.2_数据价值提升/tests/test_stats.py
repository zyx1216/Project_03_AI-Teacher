...
def test_linear_predict_three_points():
    # 严格每场 +2，下一场应为 8
    assert st.linear_predict([2, 4, 6]) == 8


def test_linear_predict_requires_three_valid_points():
    assert st.linear_predict([80, 90]) is None
    assert st.linear_predict([80, None, 90, None]) is None


def test_linear_predict_ignores_missing_values():
    assert st.linear_predict([None, 2, "", 4, 6]) == 8


def test_linear_predict_uses_only_latest_five_points():
    # 旧点形成斜率 +10；最近 5 点是稳定的 80，应预测 80，而不是沿用旧斜率。
    assert st.linear_predict([0, 10, 20, 30, 40, 80, 80, 80, 80, 80]) == 80
