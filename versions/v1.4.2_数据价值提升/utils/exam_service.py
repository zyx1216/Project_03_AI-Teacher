...
def class_trend(session, class_name: str | None = None,
    ...
    for exam in exams:
        ...
        if subject is not None:
            ...
            if not vals:
                continue
            points.append({"exam_id": exam.id, "exam_name": exam.name,
                           "values": {subject: round(sum(vals) / len(vals), 2)},
                           "full_scores": {subject: full_scores_of(exam, exam_subject_list).get(subject, 100.0)}})
            continue
        all_subjects.update(exam_subject_list)
        full_scores = full_scores_of(exam, exam_subject_list)
        point = {"exam_id": exam.id, "exam_name": exam.name, "values": {},
                 "full_scores": full_scores,
