...
def subject_summary(values, full_score: float) -> dict:
    ...
    return base
def linear_predict(values, min_points: int = 3, max_points: int = 5) -> Optional[float]:
    """
    用最近最多 max_points 个有效点做一元线性回归，预测下一场分数。
    有效点少于 min_points 时不预测；函数本身不做满分裁剪，由界面按场景裁剪。
    """
    data = clean_values(values)
