...
from utils import class_service, llm_client, backup_service, student_service
...
def _class_management_panel():
    """班级名单维护与批量调班；不新增班级表，空班级存 data/class_names.json。"""
    st.subheader("班级管理")
    st.caption("班级用于学生、作业和成绩筛选。可先预设空班级，后续导入学生时直接选择。")
    ...
    with SessionLocal() as session:
        ...
        if classes:
            rows = [{"班级": name, "学生数": usage[name]["students"],
                     "普通作业数": usage[name]["homeworks"]} for name in classes]
            st.dataframe(rows, width="stretch", hide_index=True)
        else:
            st.info("还没有班级。可先在下方添加，也可以直接导入学生名单自动生成。")

        with st.form("class_add_form"):
            new_name = st.text_input("添加班级", placeholder="如：八年级1班", key="class_new_name")
            if st.form_submit_button("➕ 添加班级", type="primary"):
                try:
                    class_service.add_class(session, new_name)
                    session.commit()
                    st.session_state["class_management_notice"] = f"已添加班级：{new_name.strip()}"
                    st.rerun()
        ...
        with c1:
            st.write("**重命名班级**")
            old_name = st.selectbox("原班级", class_options, key="class_rename_old")
            rename_name = st.text_input("新班级名称", key="class_rename_new")
            if st.button("✏️ 重命名"):
                try:
                    count = class_service.rename_class(session, old_name, rename_name)
                    session.commit()
                    st.session_state["class_management_notice"] = (
                        f"已把 {count} 名学生和相关作业的班级改为：{rename_name.strip()}")
                    st.rerun()
                except ValueError as exc:
                    st.error(str(exc))
        with c2:
            st.write("**删除空班级**")
            delete_name = st.selectbox("要删除的班级", class_options,
                                       key="class_delete_name")
            if st.button("🗑️ 删除班级", disabled=not classes):
                try:
                    class_service.delete_class(session, delete_name)
                    session.commit()
                    st.session_state["class_management_notice"] = f"已删除空班级：{delete_name}"
                    st.rerun()
                except ValueError as exc:
                    st.error(str(exc))
        ...
        picked_labels = st.multiselect("选择学生", list(student_options.keys()),
                                       key="class_move_students")
        target_class = st.selectbox("目标班级", class_options,
                                    key="class_move_target")
        if st.button("➡️ 批量调班", disabled=not classes):
            try:
                ids = [student_options[label] for label in picked_labels]
                count = class_service.move_students(session, ids, target_class)
                session.commit()
                st.session_state["class_management_notice"] = f"已调整 {count} 名学生到 {target_class}。"
                st.rerun()
            except ValueError as exc:
                st.error(str(exc))
...
def _about_panel():
    ...
    st.markdown(
        ...
        **四大模块（v1.3.0）**
        ...
        - 设置：主模型 / 内容模型 / 向量模型配置、班级管理、数据备份恢复清空
