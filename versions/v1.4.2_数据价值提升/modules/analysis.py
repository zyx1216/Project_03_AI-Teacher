...
def _student_list_panel():
    ...
    # 批量删除成功后跨 rerun 保留提示（同一帧 success+rerun 会被冲掉）。
    ...
    if students:
        ...
        if jc2.button("🔍 查看画像", use_container_width=True):
            st.session_state["analysis_tab"] = "学生画像"
            st.session_state["profile_student"] = (
                st.session_state["student_jump_profile_label"])
            st.rerun()
...
def _score_import_panel():
    """选考试 → 下载模板/上传 Excel/Word/PDF → 识别科目 → 可编辑预览 → 确认入库。"""
    # 模板先放最前面：还没建考试时老师也能先下载名单/成绩模板去填写。
...
def tab_exam_analysis():
    ...
    _term_report_export(term_filter, class_filter)
...
PREDICT_NEXT_LABEL = "下一场预测"


def _clamp_prediction(value: float | None, full_score: float | None) -> float | None:
    """预测值按最近一场满分裁剪，避免图上出现负分或超过满分。"""
    if value is None:
        return None
    if full_score and full_score > 0:
        value = min(value, full_score)
    return round(max(value, 0), 2)


def _prediction_trace(exam_names, values, full_score=None, name="预测",
                      bar_mode=False):
    """生成下一场预测 trace；有效点不足返回 None。缺考不补零。"""
    pred = _clamp_prediction(stats_mod.linear_predict(values), full_score)
    if pred is None:
        return None
    ...
    if bar_mode:
        return go.Scatter(
            x=[PREDICT_NEXT_LABEL], y=[pred],
            mode="markers+text", text=[f"{pred:g}"],
            textposition="top center", name=name,
            marker=dict(symbol="diamond", size=14, color="#d62728"),
            hovertemplate=f"{name}：{pred:g}<extra></extra>")
    return go.Scatter(
        x=[exam_names[last_i], PREDICT_NEXT_LABEL], y=[last_v, pred],
        mode="lines+markers+text", text=["", f"{pred:g}"],
        textposition="top center", name=name,
        line=dict(dash="dash", width=2, color="#d62728"),
        marker=dict(size=8), showlegend=False,
        hovertemplate="下一场预测：%{y}<extra></extra>")
...
def _term_report_export(term_filter: str, class_filter: str):
...
def tab_trends():
    ...
    show_prediction = c4.checkbox("显示下一场预测", value=True,
                                  key="trend_show_prediction")
    st.caption("预测仅供参考，实际成绩可能有波动；预测使用最近 3–5 个有效点做线性回归。")
    ...
    with st.expander("📈 班级整体趋势", expanded=True):
        _class_trend_chart(class_filter, limit, subject_arg, show_prediction)

    st.divider()
    with st.expander("🧑‍🎓 学生个人趋势", expanded=True):
        _student_trend_panel(class_filter, limit, subject_arg, show_prediction)
...
def _class_trend_chart(class_filter, limit, subject=None, show_prediction=True):
    ...
    meta = trend.get("exam_meta", [])[start:]
    x_names = exam_names + ([PREDICT_NEXT_LABEL] if show_prediction else [])

    fig = go.Figure()
    if subject is not None:
        values = trend["series"][subject][start:]
        fig.add_trace(go.Scatter(
            x=exam_names, y=values,
            mode="lines+markers", name=subject,
            line=dict(width=4, color="#1F4E79")))
        if show_prediction:
            full = (meta[-1].get("full_scores", {}).get(subject)
                    if meta else None)
            trace = _prediction_trace(x_names, values, full, name=f"{subject}预测")
            if trace is not None:
                fig.add_trace(trace)
    else:
        for item_subject in trend["subjects"]:
            values = trend["series"][item_subject][start:]
            fig.add_trace(go.Scatter(
                x=exam_names, y=values,
                mode="lines+markers", name=item_subject))
            if show_prediction:
                full = (meta[-1].get("full_scores", {}).get(item_subject)
                        if meta else None)
                trace = _prediction_trace(
                    x_names, values, full, name=f"{item_subject}预测")
                if trace is not None:
                    fig.add_trace(trace)
        total_values = trend["series"]["total"][start:]
        fig.add_trace(go.Scatter(
            x=exam_names, y=total_values,
            mode="lines+markers", name="总分",
            line=dict(width=4, dash="dot")))
        if show_prediction:
            full = meta[-1].get("total_full_score") if meta else None
            trace = _prediction_trace(x_names, total_values, full, name="总分预测")
            if trace is not None:
                fig.add_trace(trace)
...
def _student_trend_panel(class_filter, limit=None, subject=None,
    ...
    with SessionLocal() as session:
        ...
        for history_item in history:
            exam = es.get_exam(session, history_item["exam_id"])
            exam_subjects = es.exam_subjects(session, history_item["exam_id"])
            full_score_map[history_item["exam_id"]] = es.full_scores_of(
                exam, exam_subjects)
    if not history:
        st.caption("该生还没有成绩。")
        return
    ...
    if chart_type == "折线图":
        for item_subject in shown_subjects:
            values = [h.get(item_subject) for h in history]
            fig.add_trace(go.Scatter(
                x=exam_names, y=values,
                mode="lines+markers", name=item_subject, connectgaps=False))
            if show_prediction:
                full = full_score_map.get(history[-1]["exam_id"], {}).get(item_subject)
                trace = _prediction_trace(
                    x_names, values, full, name=f"{item_subject}预测")
                if trace is not None:
                    fig.add_trace(trace)
        total_values = [h.get("total") for h in history]
        fig.add_trace(go.Scatter(
            x=exam_names, y=total_values,
        ...
        if show_prediction:
            full_scores = full_score_map.get(history[-1]["exam_id"], {})
            total_full = round(sum(full_scores.values()), 2) if full_scores else None
            trace = _prediction_trace(x_names, total_values, total_full,
                                      name="总分预测")
            if trace is not None:
                fig.add_trace(trace)
    else:
        for item_subject in shown_subjects:
            ...
            fig.add_trace(go.Bar(
                x=exam_names, y=values, name=item_subject))
            if show_prediction:
                full = full_score_map.get(history[-1]["exam_id"], {}).get(item_subject)
                trace = _prediction_trace(
                    x_names, values, full, name=f"{item_subject}预测", bar_mode=True)
                if trace is not None:
                    fig.add_trace(trace)
        total_values = [h.get("total") for h in history]
        fig.add_trace(go.Bar(
