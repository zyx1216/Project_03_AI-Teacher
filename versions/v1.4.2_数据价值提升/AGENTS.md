# AI教学辅助 · AGENTS.md（v1.4.2 数据价值提升）
...
- v1.4.2 起班级不是独立表：`utils/class_service.py` 用 data/class_names.json 维护预设空班级，合并学生/作业班级；重命名同步 Student/Homework.class_name，删除只允许空班级，批量调班仍按“姓名+班级”防重。学期报告由 `term_report_service.build_term_report()` 确定性生成，Plotly 经 kaleido 导出趋势 PNG 嵌入 Word；趋势预测使用 `stats.linear_predict()`（最近 3–5 个有效点），预测只进图、不入库。
