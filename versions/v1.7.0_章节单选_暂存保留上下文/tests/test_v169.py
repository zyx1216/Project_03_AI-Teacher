...
def _select_material_chapter(at, chapter, kps):
    ...
    at.session_state["question_gen_chapter"] = chapter
...
def test_app_stash_current_question_group(tmp_path):
    ...
    # 通过控件交互更新，避免旧 Selectbox 的选项还没刷新。
    next(x for x in at.selectbox if x.key == "question_gen_material").set_value(1).run()
    next(x for x in at.selectbox if x.key == "question_gen_chapter").set_value(
        "第一单元 混合运算").run()
    next(x for x in at.multiselect if x.key == "question_gen_kps").set_value(
        ["连减运算"]).run()
