...
def _wrong_pdf_archive():
    """微软雅黑存在时嵌入；不可用则由 PyMuPDF 使用内置中文字体。"""
    import pymupdf
    ...
    if not font.exists():
        return None, """
        body { font-family: china-s, sans-serif; }
        """
...
def _wrong_pdf_groups(rows: list[dict], group_by: str):
    """按学生或知识点整理 PDF 分组；题目在同一组内去重。"""
    groups: dict[str, dict] = {}
    for r in rows:
        if group_by == "student":
            group_names = [(
                f"{r['student_name']}"
                f"（{r.get('class_name') or '未分班'}）",
                r["question_id"],
            )]
        else:
            kps = r["knowledge_points"] or [UNLABELED_KP]
            group_names = [(kp, r["question_id"]) for kp in kps]

        for group_name, qid in group_names:
            group = groups.setdefault(
                group_name, {"question_ids": [], "rows": {}})
            if qid not in group["question_ids"]:
                group["question_ids"].append(qid)
            group["rows"].setdefault(qid, r)

    return [(name, value["question_ids"], value["rows"])
            for name, value in groups.items()]


def _wrong_pdf_html(rows: list[dict], title: str, group_by: str) -> str:
    """构造错题本 PDF 的 HTML，分页由 PyMuPDF Story 自动处理。"""
    from collections import Counter
    from datetime import datetime

    if group_by not in {"student", "knowledge"}:
        raise ValueError("错题 PDF 只能按 student 或 knowledge 分组。")

    wrong_count = Counter(r["question_id"] for r in rows)
    unique_question_count = len(wrong_count)
    subject = title[:-3] if title.endswith("错题本") else title
    groups = _wrong_pdf_groups(rows, group_by)
    group_label = "学生" if group_by == "student" else "知识点"
    ...
    parts = [
        ...
        * { box-sizing: border-box; }
        ...
        .cover { height: 680px; padding-top: 150px; text-align: center;
                 page-break-after: always; }
        .cover h1 { font-size: 30pt; color: #1f4e79; margin-bottom: 20px; }
        .meta { margin-top: 40px; font-size: 12pt; color: #444;
                line-height: 2.1; }
        h2 { font-size: 16pt; color: #1f4e79; margin: 18px 0 10px;
             border-bottom: 1px solid #9eadcc; padding-bottom: 4px;
             page-break-after: avoid; }
        .question { page-break-inside: avoid; border: 1px solid #c8d4e6;
                    border-radius: 6px; padding: 8px 10px; margin: 9px 0; }
        .label { color: #1f4e79; font-weight: bold; }
        .count { color: #c00000; font-weight: bold; }
        .tip { color: #666; font-size: 9pt; margin-top: 18px; }
        """,
        "</style></head><body><div class='cover'>",
        f"<h1>{_pdf_text(title)}</h1>",
        f"<div class='meta'>学科：{_pdf_text(subject)}<br>",
        f"导出时间：{datetime.now().strftime('%Y-%m-%d %H:%M')}<br>",
        f"错题记录：{len(rows)} 条<br>",
        f"涉及题目：{unique_question_count} 道<br>",
        f"分类方式：按{group_label}</div></div>",
    ]

    for group_name, question_ids, question_rows in groups:
        parts.append(f"<h2>{_pdf_text(group_name)}</h2>")
        for index, qid in enumerate(question_ids, start=1):
            r = question_rows[qid]
            parts.append("<div class='question'>")
            parts.append(f"<div><span class='label'>{index}. 题目：</span>"
                         f"{_pdf_text(r['content'])}</div>")
            parts.append(f"<div><span class='label'>正确答案：</span>"
                         f"{_pdf_text(r['answer'])}</div>")
            if r.get("analysis"):
                parts.append(f"<div><span class='label'>解析：</span>"
                             f"{_pdf_text(r['analysis'])}</div>")
            parts.append("<div><span class='label'>错误次数：</span>"
                         f"<span class='count'>{wrong_count[qid]}</span></div>")
            parts.append("</div>")

    parts.append("<div class='tip'>本 PDF 由当前错题筛选结果生成。</div>")
    parts.append("</body></html>")
    return "".join(parts)


def export_wrong_pdf(rows: list[dict], title: str = "错题本",
                     group_by: str = "student") -> bytes:
    """把错题导出为 PDF；group_by 支持 student / knowledge。"""
    import io
    import pymupdf

    archive, font_css = _wrong_pdf_archive()
    html = _wrong_pdf_html(rows, title, group_by).replace(
