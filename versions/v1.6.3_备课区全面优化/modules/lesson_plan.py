...
def tab_lesson():
    ...
    if material_id is not None:
        ...
        if path.exists():
            chapter_options = [
                c["title"] for c in material.split_chapters(path.read_text(encoding="utf-8"))]
    chapter_pick = ch_c1.multiselect(
        "章节筛选（从资料章节选）", chapter_options,
        key="lesson_chapter_pick")
    chapter_manual = ch_c2.text_input(
        "章节（可手动修改）", key="lesson_chapter_manual")
    chapter = (chapter_manual.strip()
               or "、".join(chapter_pick))
