...
def tab_materials():
    ...
    grade_filter = top_c2.selectbox("年级筛选", ["全部年级"] + GRADE_CHOICES)
...
def _preview_pending():
    ...
    if st.button("💾 保存资料", key="save_material", type="primary"):
        ...
        st.session_state.pop("mt_pending", None)
        st.success(f"资料已保存（id={saved_id}）。可打开原版、查看详情或建立向量索引。")
        st.rerun()
        ...
        for tb in books:
            with st.container(border=True):
                ...
                if original_url:
                    ...
                    c1.link_button(
                        tb.name, original_url, key=f"material_open_{tb.id}",
...
def tab_lesson():
    ...
    grade = row1.selectbox("年级", GRADE_CHOICES[:-1], index=7,
    ...
    template_name = row3.selectbox("教案模板", template_names,
    ...
    if st.button("🤖 生成教案", type="primary", key="generate_lesson_button"):
        ...
        if not topic.strip():
            ...
            st.warning("请先在“章节搜索与选择”中勾选要使用的章节。")
...
def _lesson_material_picker(subject):
    ...
    with st.container(border=True):
        st.markdown("**章节搜索与选择**")
        keyword = st.text_input("输入关键词筛选章节", key="lesson_chapter_keyword")
        with SessionLocal():
        ...
        selected = st.multiselect(
            "勾选后作为教案上下文", titles, default=default,
            key="lesson_chapter_select")
        st.caption(f"筛选结果 {len(titles)} 个，已选 {len(selected)} 个。")
...
def tab_question_gen():
    ...
    drafts = st.session_state.setdefault(qs.DRAFTS_STATE_KEY, {})
...
def _save_previous_question_draft():
    ...
    try:
        ...
        st.session_state.setdefault(qs.DRAFTS_STATE_KEY, {})[old_subject] = draft
    finally:
