...
def test_export_word_explicit_subject_title(session):
    from docx import Document

    qi.import_records(session, [_valid_record("物理题干", "物理答案")], subject="物理")
    question = qs.list_questions(session, subject="物理")[0]
    assert question.subject == "物理"

    data = qs.export_questions_word(
        [question], with_answer=False, title="物理习题")
    title = Document(io.BytesIO(data)).paragraphs[0].text

    assert title == "物理习题"


def test_export_word_generic_default_title(session):
    from docx import Document

    qi.import_records(session, [_valid_record("题干", "答案")])
    question = qs.list_questions(session)[0]
    data = qs.export_questions_word([question], with_answer=False)
    title = Document(io.BytesIO(data)).paragraphs[0].text
