...
def _new_homework_dialog():
    ...
    st.markdown("**选择作业类型**")
    cols = st.columns(5)
    chosen = st.session_state.get("hw_new_type", "after_class")
    for i, key in enumerate(_TYPE_KEYS):
        active = key == chosen
        # 选中态蓝色高亮（type=primary），未选用普通按钮
        if cols[i].button(
    ...
    with st.form("new_homework_form"):
        ...
        total_score = c2.number_input("总分", min_value=1.0, value=float(defaults["total_score"]),
                                      step=5.0)
        duration = c3.number_input("建议用时（分钟）", min_value=1,
                                   value=int(defaults["duration"]), step=5)
    ...
    if submitted:
        if not name.strip():
            ...
            with SessionLocal() as session:
                ...
                session.commit()
                st.session_state["hw_open_id"] = hw.id
            st.rerun()
    if cancelled:
...
def tab_manage():
    ...
    if st.button("➕ 新建作业 / 试卷", type="primary"):
        st.session_state["hw_dialog_open"] = True
...
def _generate_and_add(session, hw, knowledge, types, difficulty, count):
    ...
    new_ids = []
    for data in valid:
        q = qs.create_question(session, data, source="ai_generated", status="pending")
        new_ids.append(q.id)
    session.flush()
...
def _pick_homework_and_class(templates: bool = False):
    ...
    labels = [f"{hw_svc.type_emoji(hw.homework_type)} {hw.name}（{hw.class_name or '未分班'}）"
              for hw in homeworks]
...
def tab_wrong_book():
    ...
    if st.button("📄 导出错题本 Word", key="wb_export"):
        data = hscore.export_wrong_word(rows, title="数学错题本")
        st.download_button("⬇️ 下载错题本", data, file_name="错题本.docx",
...
def show() -> None:
    """作业工作台入口：4 个页内标签页。"""
    st.title("📝 作业工作台")
    t1, t2, t3, t4 = st.tabs(["作业管理", "成绩录入", "作业分析", "错题本"])
    with t1:
        tab_manage()
    with t2:
        tab_scores()
