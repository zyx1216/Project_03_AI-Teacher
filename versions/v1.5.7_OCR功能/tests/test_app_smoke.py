...
def test_scanned_pdf_ocr_preview_isolated(tmp_path):
    """扫描件 PDF 自动 OCR，成功后进入章节预览。"""
    db_file = tmp_path / "ocr_success.db"
    at = AppTest.from_string(
        _isolated_app_code(db_file, tmp_path / "feature.json", ocr_mode="success"),
        default_timeout=30)
    at.run()
    at.sidebar.radio[0].set_value("📚 备课").run()
    ...
    assert not at.exception, [str(e) for e in at.exception]
    assert any("检测到这是扫描件PDF" in str(x.value) for x in at.info)
    assert any("OCR识别可能有误差" in str(x.value) for x in at.info)
    assert any("提取成功" in str(x.value) for x in at.success)
    assert any(b.label == "💾 保存资料" for b in at.button)


def test_scanned_pdf_ocr_failure_shields_user_isolated(tmp_path):
    """OCR 失败时显示固定中文错误，不出现白屏异常。"""
    db_file = tmp_path / "ocr_failure.db"
    at = AppTest.from_string(
        _isolated_app_code(db_file, tmp_path / "feature.json", ocr_mode="failure"),
        default_timeout=30)
    at.run()
    at.sidebar.radio[0].set_value("📚 备课").run()
    ...
    assert not at.exception, [str(e) for e in at.exception]
    assert any(
        "OCR识别失败，请检查PDF质量，或改用文字版PDF/Word" == str(x.value)
        for x in at.error)
