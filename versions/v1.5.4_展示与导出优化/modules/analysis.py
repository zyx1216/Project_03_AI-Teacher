...
def _student_list_panel():
    ...
    st.caption(f"共 {len(students)} 名学生。可直接改表格，也可用最后一行新增学生；改完点下方“保存表格修改”。")
    if initial_df.empty:
...
def tab_scores():
    ...
    import_expanded = bool(st.session_state.get("score_import_expander"))
    with st.expander("📥 从 Excel / Word / PDF 导入成绩", expanded=import_expanded):
        _score_import_panel()
...
def tab_knowledge():
    ...
    if not homeworks:
        st.info("还没有普通作业；先到「📝 作业」新建作业并布置给学生。")
        return
    ...
    if not rows:
        st.info("这份作业还没有逐题批改数据；先到「📝 作业 → 成绩录入」逐题打分。")
        return
