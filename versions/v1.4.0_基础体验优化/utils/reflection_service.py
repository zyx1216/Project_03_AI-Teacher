...
def _exam_metrics_lines(session, exam: Exam, class_name: str | None) -> list[str]:
    """一场考试的指标文本行（复用考试分析口径）。"""
    data = exam_service.analyze_exam(session, exam.id, class_name)
    ...
    for subject in data["subjects"]:
        ...
        lines.append(
            ...
            f"均分{stt['mean']}，中位数{stt['median']}，最高{stt['max']:g}，"
...
def build_reflection_data_text(session, scope_type: str, start_exam_id: int,
    ...
    lines = [f"反思范围：{scope_label}",
             ...
             "【一、考试成绩数据】"]
    for exam in exams:
        lines.extend(_exam_metrics_lines(session, exam, class_name))
