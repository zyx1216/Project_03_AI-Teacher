...
def tab_materials():
    ...
    with st.form("upload_material"):
        name = st.text_input("资料名称", placeholder=f"如：人教版八年级上册{current_subject} 第X章")
            ...
            if file_type == "pdf":
                if uploaded is None:
                    ...
                    if ocr_service.is_scanned_pdf(pdf_bytes):
                        ...
                        ocr_tasks.start_ocr_task(
                            pdf_bytes=pdf_bytes,
                            name=(name or uploaded.name).strip() or uploaded.name,
                            subject=current_subject,
                            grade=grade,
                            source_name=uploaded.name,
                            session_factory=SessionLocal,
                            text_dir=TEXT_DIR)
                        st.success("OCR 任务已开始；切换页面不会中断，下方显示识别进度。")
                    else:
                        text = material.extract_pdf(pdf_bytes)
                        st.session_state["mt_pending"] = (
                            name or uploaded.name, file_type, grade, uploaded.name,
                ...
                if existing_name:
                    ...
                    st.session_state["mt_pending"] = (
                        ...
                        normalized_url, source_text, current_subject)
            else:
...
@st.fragment(run_every=2, key="ocr_task_status_fragment")
def _render_ocr_tasks():
    """后台 OCR 任务状态；每 2 秒自动刷新一次。"""
    tasks = ocr_tasks.list_tasks()
    if not tasks:
        return

    refreshed = st.session_state.setdefault("_ocr_completed_refreshed", [])
    newly_completed = [
        task for task in tasks
        if task["status"] == ocr_tasks.STATUS_COMPLETED
        and task["id"] not in refreshed
    ]
    if newly_completed:
        refreshed.extend(task["id"] for task in newly_completed)
        st.rerun()
...
def _list_materials():
    """已保存资料列表 + 向量化按钮（按资料管理学科过滤，兼容无学科旧资料）。"""
    current_subject = fs.get_feature_subject(fs.MATERIALS_SUBJECT)
    with SessionLocal() as session:
        books = (session.query(Textbook)
                 .filter((Textbook.subject == current_subject) |
                         (Textbook.subject.is_(None)))
                 .order_by(Textbook.id.desc()).all())
        if not books:
            st.info(f"当前学科「{current_subject}」还没有资料。可上传 PDF/Word、粘贴文本，或输入公开网页链接。")
            return
        for tb in books:
...
def tab_lesson():
    ...
    with col_form:
        with st.form("lesson_params", border=True):
            st.markdown("**① 备课参数**")
            current_subject = _subject_selectbox(fs.LESSON_PLAN_SUBJECT, in_form=True)
            ...
            if books:
                ...
                st.info(
                    f"当前学科「{current_subject}」还没有教材资料；先到「资料管理」"
                    "上传 PDF/Word、粘贴文本或添加网页链接，生成教案效果更好。"
                    "也可以不使用资料直接生成。")
    ...
    if generate:
        ...
        if not topic.strip():
            ...
            _generate_lesson(topic.strip(), grade.strip(), chapter.strip(),
                             hours, style, tb_choice, current_subject)
...
def _generate_lesson(topic, grade, chapter, hours, style, textbook_id, subject):
...
def _saved_lessons():
    """已保存教案列表：载入编辑 / 导出 Word / 删除（按 AI 备课学科过滤）。"""
...
def tab_question_gen():
    ...
    with st.form("question_gen_form", border=True):
        current_subject = _subject_selectbox(fs.QUESTION_GEN_SUBJECT, in_form=True)
    ...
    if generate:
        fs.set_feature_subject(fs.QUESTION_GEN_SUBJECT, current_subject)
        if not knowledge.strip():
            st.warning("请先填写知识点。")
        elif not llm_client.is_content_configured():
            st.warning("还没配置 AI：请到「⚙️ 设置」填写 API Key 和内容生成模型。")
        else:
            _generate_questions(knowledge.strip(), types, difficulty, count,
                                extra.strip(), current_subject)

    if "gen_questions" in st.session_state:
        _preview_generated()

def _generate_questions(knowledge, types, difficulty, count, extra, subject):
...
def _preview_generated():
    ...
    if c1.button("✅ 确认入库（进入待审核）", type="primary"):
        with SessionLocal() as session:
            for q in questions:
                subject = st.session_state.get("gen_subject", DEFAULT_SUBJECT)
                qs.create_question(session, q, source="ai_generated",
        ...
        st.session_state.pop("gen_subject", None)
        st.rerun()
    if c2.button("清空本次结果"):
        st.session_state.pop("gen_questions", None)
        st.session_state.pop("gen_rejected", None)
...
def tab_ppt():
    ...
    with SessionLocal() as session:
        plans = lesson_svc.list_plans(session, subject=current_subject)
        if not plans:
            st.info(f"当前学科「{current_subject}」还没有已保存教案。请先到“AI 备课”生成并保存一份。")
            return
