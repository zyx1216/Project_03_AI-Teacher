...
import config
from utils import llm_client, backup_service, app_config
...
def _current_subject_panel():
    """当前学科选择（v1.2.4 恢复）：只影响数据归属与过滤，不影响标题/图标。"""
    st.subheader("当前学科")
    st.caption("选择你当前任教的学科，会影响资料、题库、作业的归属与过滤，不影响应用标题。已有的成绩数据不受影响。")
    current = app_config.get_current_subject()
    idx = app_config.ALL_SUBJECTS.index(current) if current in app_config.ALL_SUBJECTS else 1
    chosen = st.selectbox("当前学科", app_config.ALL_SUBJECTS,
                          index=idx, key="settings_current_subject")
    if chosen != current:
        app_config.set_current_subject(chosen)
        st.success(f"学科已切换为「{chosen}」，资料/题库/作业列表已按该学科过滤。")
...
def _about_panel():
    ...
    st.markdown(
        ...
        **四大模块（v1.2.4）**
        ...
        - 设置：主模型 / 内容模型 / 向量模型配置、数据备份恢复清空、当前学科
        - 共 13 张数据表；切换学科只影响资料/题库/作业的归属与过滤，应用标题保持固定；详细操作见项目根目录的 `USAGE.md`。
