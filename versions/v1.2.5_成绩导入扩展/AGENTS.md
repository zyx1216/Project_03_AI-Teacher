# AI教学辅助 · AGENTS.md（v1.2.5 成绩导入扩展）

> 本文件只在本项目目录内生效；全局规则见 `~/.codex/AGENTS.md`，此处不复制。

## 项目概览
- 目标：单人中小学教师本地使用的 AI 教学辅助（备课 / 作业 / 学情分析 / 设置），v1.2 起支持语文、数学、英语、物理、化学、生物、政治、历史、地理 9 学科全局切换。
...
## 架构约定
- 4 个侧边栏页面：备课(modules/lesson_plan.py，5 tab，资料支持 PDF/Word/文本/网页链接)、作业(homework.py，4 tab)、学情(analysis.py，7 tab)、设置(settings.py，含数据管理区和当前学科选择器)。每页内部用 st.tabs，每 tab 一个函数，show() 只路由。新建作业、新建考试都用 @st.dialog 弹窗；@st.dialog 必须贴在真正的弹窗内容函数上，绝不能贴在 on_click 回调上（否则回调被当成新弹窗，点类型弹空窗）；弹窗打开状态持久化到 st.session_state，弹窗内“改 state 重渲染内容”用 on_click 回调、回调外不手动 st.rerun()，只有关闭弹窗才整页 rerun。学生表格 data_editor 用固定 key（不拼筛选条件，否则组件重建触发 DOM 报错），筛选/搜索变化时主动 pop 该 key 重置；用隐藏“学生ID”列映射数据库记录，不能依赖表格行号。
- 当前学科配置在 `utils/app_config.py`，运行时文件为 `data/app_config.json`（v1.2.4 起设置页恢复学科选择器）。**学科只决定数据归属/过滤，标题/图标始终固定「📐 AI教学辅助」不随学科变**。资料/题库/作业/错题都按当前学科过滤；服务层过滤参数默认 None=不过滤（保证旧调用兼容）；Textbook 过滤口径 `subject==当前 OR subject IS NULL`，Question/Homework 因有 DEFAULT '数学' 直接等值过滤。过滤相关页面控件随视角切换时用固定 key（动态 key 会产生孤儿控件状态）。
...
## 测试与验证
- tests/：test_stats.py（统计口径）、test_excel_import.py/test_score_doc_parser.py（Excel/Word/PDF 识别→可编辑预览→入库→分析全链路）、test_app_smoke.py（AppTest 4 主页 + 学情 7 tab，空库/有数据用临时 SQLite+runpy 隔离引擎；学科 UI 用临时 app_config.json；AppTest 中 data_editor 出现在 at.dataframe）。
