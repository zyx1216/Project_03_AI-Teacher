...
def _isolate_global_app_config(tmp_path, monkeypatch):
    """所有页面冒烟测试都把全局学科配置指到临时文件。"""
    from utils import app_config
    monkeypatch.setattr(app_config, "APP_CONFIG_PATH", tmp_path / "app_config.json")
...
def test_settings_subject_selector_but_title_fixed_isolated(tmp_path):
    """v1.2.4：设置页恢复学科选择器（只过滤数据）；标题/图标仍固定，不随学科变。"""
    db_file = tmp_path / "settings.db"
    app_config_file = tmp_path / "settings_app_config.json"
    app_config_file.write_text(
        '{"current_subject": "物理"}', encoding="utf-8")
    at = AppTest.from_string(
        _isolated_app_code(db_file, app_config_file), default_timeout=30)
    at.run()
    assert not at.exception, [str(e) for e in at.exception]
    # 标题/图标固定，即使当前学科是物理也不显示 ⚛️
    assert any(x.value == "📐 AI教学辅助" for x in at.sidebar.title)

    at.sidebar.radio[0].set_value("⚙️ 设置").run()
    assert not at.exception, [str(e) for e in at.exception]
    # 恢复“当前学科”选择器，当前值为物理
    subject_box = next(x for x in at.selectbox if x.label == "当前学科")
    assert subject_box.value == "物理"
    # 切换学科后写入配置，标题仍固定
    subject_box.set_value("化学").run()
    assert not at.exception, [str(e) for e in at.exception]
    assert any(x.value == "📐 AI教学辅助" for x in at.sidebar.title)
    assert app_config_file.read_text(encoding="utf-8").find("化学") >= 0
...
def test_homework_new_dialog_decorator_on_content_function():
    """v1.2.3 回归：@st.dialog 必须贴在弹窗内容函数上，不能贴在类型回调上。

    AppTest 抓不到“装饰器贴错导致点类型弹空窗”，这里用 __wrapped__ 标记静态守住。
    """
    import modules.homework as hw_module
    # 被 @st.dialog 装饰的函数带 __wrapped__；普通回调函数没有。
    assert hasattr(hw_module._new_homework_dialog, "__wrapped__"), \
        "_new_homework_dialog 必须是真正的 @st.dialog 弹窗"
    assert not hasattr(hw_module._choose_new_homework_type, "__wrapped__"), \
        "类型选择回调不能加 @st.dialog，否则点类型会弹出一个空弹窗"


def test_homework_new_dialog_persists_switch_and_creates_isolated(tmp_path):
    """弹窗内切换类型不闪退、不弹空窗；试卷默认 120/90；创建真实写入临时库。"""
    db_file = tmp_path / "homework_dialog.db"
    at = AppTest.from_string(
        _isolated_app_code(db_file, tmp_path / "homework_dialog_app_config.json"),
        default_timeout=30)
    at.run()
    at.sidebar.radio[0].set_value("📝 作业").run()
    ...
    # 先切预习，再切试卷：每次 rerun 后弹窗仍在，说明持久状态生效。
    ...
    assert total_input.value == 120.0
    assert duration_input.value == 90
    ...
    hw = session.query(Homework).one()
    assert (hw.name, hw.homework_type, hw.total_score, hw.duration) == (
        "临时弹窗试卷", "exam", 120.0, 90)
    session.close()
    eng.dispose()
...
def _isolated_app_code(db_file: Path, app_config_file: Path | None = None) -> str:
    """生成在临时数据库和临时学科配置上跑 app.py 的脚本。"""
    app_config_file = app_config_file or db_file.parent / "app_config.json"
    template = '''
import sys, runpy
...
app_config.APP_CONFIG_PATH = _Path(r"__APPCONFIG__")
...
'''
    return (template.replace("__ROOT__", str(_PROJECT_ROOT))
            ...
            .replace("__APPCONFIG__", app_config_file.as_posix())
...
def test_reflection_comments_empty_state_isolated(tmp_path):
    ...
    app_config_file = tmp_path / "empty_app_config.json"
    at = AppTest.from_string(_isolated_app_code(db_file, app_config_file), default_timeout=30)
    at.run()
    at.sidebar.radio[0].set_value("📊 学情").run()
    assert not at.exception, [str(e) for e in at.exception]
...
def test_reflection_and_comments_with_data_isolated(tmp_path, monkeypatch):
    ...
    at = AppTest.from_string(_isolated_app_code(db_file, tmp_path / "reflection_app_config.json"), default_timeout=30)
    at.run()
    at.sidebar.radio[0].set_value("📊 学情").run()
    assert not at.exception, [str(e) for e in at.exception]
    ...
    # ---- 期末评语：批量生成（2 人）后一键保存 ----
    at2 = AppTest.from_string(_isolated_app_code(db_file, tmp_path / "with_data_app_config.json"), default_timeout=30)
    at2.run()
...
def test_new_exam_dialog_full_score_rows_isolated(tmp_path):
    ...
    at = AppTest.from_string(
        _isolated_app_code(db_file, tmp_path / "exam_dialog_app_config.json"),
        default_timeout=30)
    at.run()
    at.sidebar.radio[0].set_value("📊 学情").run()
...
def test_student_editor_and_trend_controls_isolated(tmp_path):
    ...
    eng = _seed_temp_db(db_file)
    at = AppTest.from_string(
        _isolated_app_code(db_file, tmp_path / "student_trend_app_config.json"),
        default_timeout=30)
    at.run()
    at.sidebar.radio[0].set_value("📊 学情").run()
    assert not at.exception, [str(e) for e in at.exception]
    ...
    # v1.2.4：趋势有了科目选择器，默认当前学科“数学”，个人图多选默认只勾数学
    trend_subject_box = next(x for x in at.selectbox if x.label == "科目")
    assert trend_subject_box.value == "数学"
...
def _seed_wrong_answer_isolated(db_file: Path, app_config_file: Path):
    """在临时库造 1 条错题，供错题本按当前学科导出的页面冒烟。"""
    from sqlalchemy import create_engine
    from sqlalchemy.orm import sessionmaker
    from models.models import Base
    from utils import app_config, homework_service, homework_score_service
    from utils import question_service, student_service

    app_config.APP_CONFIG_PATH = app_config_file
    app_config.set_current_subject("物理")

    eng = create_engine(f"sqlite:///{db_file.as_posix()}")
    Base.metadata.create_all(eng)
    session = sessionmaker(bind=eng)()
    homework = homework_service.create_homework(session, "物理作业", class_name="冒烟班")
...
def test_wrong_book_export_uses_current_subject_isolated(tmp_path):
    ...
    app_config_file = tmp_path / "wrong_book_app_config.json"
    _seed_wrong_answer_isolated(db_file, app_config_file)
...
def test_lesson_materials_web_link_empty_url_isolated(tmp_path):
    ...
    at = AppTest.from_string(_isolated_app_code(db_file, tmp_path / "web_app_config.json"), default_timeout=30)
    at.run()
    at.sidebar.radio[0].set_value("📚 备课").run()
    assert not at.exception, [str(e) for e in at.exception]
...
def test_lesson_materials_web_link_preview_and_save_isolated(tmp_path, monkeypatch):
    ...
    app_config_file = tmp_path / "web_save_app_config.json"
    app_config_file.write_text(
        '{"current_subject": "物理"}', encoding="utf-8")
...
def test_lesson_materials_filtered_by_current_subject_isolated(tmp_path):
    ...
    app_config_file = tmp_path / "subject_lesson_app_config.json"
    app_config_file.write_text('{"current_subject": "物理"}', encoding="utf-8")
...
def test_question_bank_filtered_by_current_subject_isolated(tmp_path):
    ...
    app_config_file = tmp_path / "subject_bank_app_config.json"
    app_config_file.write_text('{"current_subject": "物理"}', encoding="utf-8")
    _seed_subject_isolated_db(db_file)
...
def test_homework_list_filtered_by_current_subject_isolated(tmp_path):
    ...
    app_config_file = tmp_path / "subject_hw_app_config.json"
    app_config_file.write_text('{"current_subject": "数学"}', encoding="utf-8")
    _seed_subject_isolated_db(db_file)
...
def test_exam_analysis_subject_view_switch_isolated(tmp_path):
    ...
    app_config_file = tmp_path / "subject_exam_app_config.json"
    app_config_file.write_text('{"current_subject": "数学"}', encoding="utf-8")
    _seed_subject_isolated_db(db_file)
...
def test_score_import_accepts_docx_and_editable_preview_isolated(tmp_path):
    ...
    at = AppTest.from_string(
        _isolated_app_code(db_file, tmp_path / "score_docx_app_config.json"),
        default_timeout=30)
    at.run()
    at.sidebar.radio[0].set_value("📊 学情").run()
    assert not at.exception, [str(e) for e in at.exception]
...
def test_score_import_accepts_pdf_isolated(tmp_path):
    ...
    _seed_temp_db(db_file)
    at = AppTest.from_string(
        _isolated_app_code(db_file, tmp_path / "score_pdf_app_config.json"),
        default_timeout=30)
    at.run()
    at.sidebar.radio[0].set_value("📊 学情").run()
...
def test_score_import_complex_pdf_friendly_hint_isolated(tmp_path):
    ...
    _seed_temp_db(db_file)
    at = AppTest.from_string(
        _isolated_app_code(db_file, tmp_path / "score_pdf_hint_app.json"),
        default_timeout=30)
    at.run()
    at.sidebar.radio[0].set_value("📊 学情").run()
    _import_expander(at).file_uploader[0].upload(
...
def test_trend_range_default_and_chart_switch_isolated(tmp_path):
    ...
    _seed_temp_db(db_file)
    at = AppTest.from_string(
        _isolated_app_code(db_file, tmp_path / "trend_v125_app.json"),
        default_timeout=30)
    at.run()
    at.sidebar.radio[0].set_value("📊 学情").run()
    assert not at.exception, [str(e) for e in at.exception]
