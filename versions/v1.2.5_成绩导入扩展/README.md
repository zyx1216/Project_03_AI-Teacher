...
面向**单人中小学教师**本地运行的 AI 辅助教学工具，集备课、作业、学情分析于一体；底层支持 9 个学科，v1.2.4 起可在设置页切换当前学科（资料/题库/作业按学科过滤与归属），v1.2.5 起成绩支持 Excel/Word/PDF 导入；界面标题固定为“AI教学辅助”、不随学科变化。
...
生成的 Excel 在 `data/uploads/demo/`，到「📊 学情」依次导入名单和三场成绩即可看到完整分析。
...
| `modules/` | 页面模块：analysis（学情，7 tab）、lesson_plan（备课，5 tab）、homework（作业，4 tab）、settings |
...
| `utils/app_config.py` | 当前学科配置（9 学科，写入 data/app_config.json；v1.2.4 起设置页可切换，只过滤数据，标题固定） |
| `data/` | 运行时数据（库、上传、导出、向量库、llm_config.json、app_config.json），已被 git 忽略 |
