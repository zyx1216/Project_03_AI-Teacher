...
def test_v141_question_batch_approve_isolated(tmp_path):
    ...
    # v1.6.3：批量审核收到表格上方按钮后才展开
    next(b for b in at.button if b.key == "bank_batch_approve_top").click().run()
    ...
    next(b for b in at.button if b.key == "bank_batch_approve_btn").click().run()
    assert not at.exception, [str(e) for e in at.exception]
