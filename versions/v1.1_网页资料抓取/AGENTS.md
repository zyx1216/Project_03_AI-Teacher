...
## 架构约定
- 4 个侧边栏页面：备课(modules/lesson_plan.py，5 tab，资料支持 PDF/Word/文本/网页链接)、作业(homework.py，4 tab)、学情(analysis.py，7 tab)、设置(settings.py，含数据管理区)。每页内部用 st.tabs，每 tab 一个函数，show() 只路由。新建作业、新建考试都用 @st.dialog 弹窗。
