...
def test_settings_has_no_subject_selector_and_title_fixed_isolated(tmp_path):
    """v1.2.3：设置页不再有学科切换；即使配置是物理，侧边栏标题仍固定。"""
    ...
    at.run()
    assert not at.exception, [str(e) for e in at.exception]
    # 标题/图标固定，不随配置里的物理变成 ⚛️
    assert any(x.value == "📐 AI教学辅助" for x in at.sidebar.title)

    at.sidebar.radio[0].set_value("⚙️ 设置").run()
    assert not at.exception, [str(e) for e in at.exception]
    # 不再出现“当前学科”选择器和切换提示
    assert not any(x.label == "当前学科" for x in at.selectbox)
    captions = " ".join(str(x.value) for x in at.caption)
    assert "选择你当前任教的学科" not in captions
    assert not any("学科已切换" in str(x.value) for x in at.success)
