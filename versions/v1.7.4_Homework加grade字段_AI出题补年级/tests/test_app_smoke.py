...
def test_app_starts_and_renders_default_page(tmp_path):
    ...
    assert at.sidebar.radio[0].options == ["🏠 首页", "📅 教学日历", "📚 备课", "📝 学业测评", "📊 学情", "⚙️ 设置"]
...
def test_switch_all_main_pages(tmp_path):
    at = AppTest.from_string(
        ...
        default_timeout=30)
    at.run()
    for page in ["📅 教学日历", "📚 备课", "📝 学业测评", "📊 学情", "⚙️ 设置"]:
...
def test_lesson_page_has_five_tabs_empty_state(tmp_path):
    ...
    # v1.6.2 起出题页学科单选切换；默认学科仍为数学。
    subject_picker = next(x for x in at.radio if x.key == "question_gen_subject")
...
def _isolated_app_code(db_file: Path, feature_file: Path | None = None,
                       ...
                       ocr_source_dir: Path | None = None,
                       original_dir: Path | None = None,
                       static_original_dir: Path | None = None,
                       ...
                       question_drafts_file: Path | None = None,
                       question_tasks_file: Path | None = None,
                       pending_groups_file: Path | None = None,
    ...
    ocr_source_dir = ocr_source_dir or db_file.parent / "ocr_source"
    original_dir = original_dir or db_file.parent / "original"
    static_original_dir = static_original_dir or db_file.parent / "static_original"
    ...
    question_drafts_file = question_drafts_file or db_file.parent / "question_drafts.json"
    question_tasks_file = question_tasks_file or db_file.parent / "question_tasks.json"
    pending_groups_file = pending_groups_file or db_file.parent / "pending_question_groups.json"
...
ocr_task_service.OCR_SOURCE_DIR = _Path(r"__OCRSOURCEDIR__")
material_service.OCR_SOURCE_DIR = _Path(r"__OCRSOURCEDIR__")
material_service.ORIGINAL_DIR = _Path(r"__ORIGINALDIR__")
material_service.STATIC_ORIGINAL_DIR = _Path(r"__STATICORIGINALDIR__")
...
question_service.DRAFTS_FILE_PATH = _Path(r"__QUESTIONDRAFTSFILE__")
question_service.QUESTION_TASKS_PATH = _Path(r"__QUESTIONTASKSFILE__")
question_service.PENDING_GROUPS_PATH = _Path(r"__PENDINGGROUPSFILE__")
...
"""
    return (template.replace("__ROOT__", str(_PROJECT_ROOT))
            ...
            .replace("__OCRSOURCEDIR__", ocr_source_dir.as_posix())
            .replace("__ORIGINALDIR__", original_dir.as_posix())
            .replace("__STATICORIGINALDIR__", static_original_dir.as_posix())
            ...
            .replace("__QUESTIONDRAFTSFILE__", question_drafts_file.as_posix())
            .replace("__QUESTIONTASKSFILE__", question_tasks_file.as_posix())
            .replace("__PENDINGGROUPSFILE__", pending_groups_file.as_posix())
...
def test_lesson_materials_web_link_renders_with_data_isolated(tmp_path):
    ...
    # AppTest 1.64 未暴露 link_button；名称入口由服务层测试与无异常覆盖，
    # 这里继续断言网络类型、URL 和保留操作。
    assert "网络导入" in caption_text
    assert "https://example.com/math-article" in caption_text
    assert any(b.label == "建立索引" for b in at.button)
...
def test_question_bank_filtered_by_current_subject_isolated(tmp_path):
    ...
    # v1.6.3 起题目以 AgGrid 表格展示；AppTest 不跑自定义 JS，
    # 这里只断言表格挂载、没有空状态提示、数学题被过滤
    assert not any("题库里还没有符合条件的题" in str(x.value) for x in at.info)
    assert not any("题目表格加载失败" in str(x.value) for x in at.error)
...
def test_v141_question_batch_approve_isolated(tmp_path):
    ...
    # 当前批量操作直接读取 AgGrid 勾选行；AppTest 不跑前端，先注入组件返回再触发点击。
    at.session_state["question_bank_grid"] = {
        "nodes": [
            {"data": {"ID": 1, "question_id": 1}, "isSelected": True},
            {"data": {"ID": 2, "question_id": 2}, "isSelected": True},
        ],
        "gridOptions": {},
    }
    approve_btn = next(b for b in at.button if b.key == "bank_batch_approve_bottom")
    approve_btn.click()
    at.run()
    assert not at.exception, [str(e) for e in at.exception]
...
def test_v153_dashboard_quick_action_new_homework(tmp_path):
    ...
    assert not at.exception, [str(e) for e in at.exception]
    assert at.sidebar.radio[0].value == "📝 学业测评"
...
def test_v153_retry_homework_generate_isolated(tmp_path):
    ...
    assert new_hw["name"] == "错题重做卷_" + _dt.now().strftime("%Y%m%d")
