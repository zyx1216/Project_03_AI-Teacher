...
def test_grade_mapping_primary_unchanged_and_senior_mapped():
    for name in ("一年级", "六年级", "未指定"):
        assert ac.to_display_grade(name) == name
        assert ac.to_storage_grade(name) == name
    assert ac.to_storage_grade("高一") == "十年级"
    assert ac.to_display_grade("十一年级") == "高二"
...
def test_bank_table_rows_shape(session):
    ...
    assert set(rows[0]) >= {"ID", "题型", "难度", "状态", "知识点",
