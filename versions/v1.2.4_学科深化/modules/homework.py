...
def _new_homework_dialog():
    ...
    with st.form("new_homework_form"):
        ...
        with SessionLocal() as session:
            templates = hw_svc.list_homeworks(
                session, templates=True, subject=app_config.get_current_subject())
        template_id = None
        if templates:
    ...
    if submitted:
        if not name.strip():
            ...
            _clear_new_homework_widget_keys()
            st.rerun()
    if cancelled:
        _clear_new_homework_widget_keys()
        st.rerun()

def _clear_new_homework_widget_keys():
    """关闭新建弹窗后清掉类型、总分、用时状态，下次打开重新取默认值。"""
...
def _add_from_bank(session, hw):
    ...
    with st.container(border=True):
        ...
        questions = qs.list_questions(session, question_type=ftype,
                                      ...
                                      subject=app_config.get_current_subject())
...
def _pick_homework_and_class(templates: bool = False):
    ...
    with SessionLocal() as session:
        homeworks = hw_svc.list_homeworks(
            session, templates=False, subject=app_config.get_current_subject())
