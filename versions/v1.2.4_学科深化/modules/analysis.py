...
def _new_exam_dialog():
    ...
    if rows_key not in st.session_state:
        st.session_state[rows_key] = [
            {"rid": 1, "subject": app_config.get_current_subject(), "score": 100.0}
        ]
...
def _initial_exam_full_rows(exam, subjects: list[str]) -> list[dict]:
    ...
    if not subject_names:
        subject_names = [app_config.get_current_subject()]
    return [
...
def tab_exam_analysis():
    ...
    # 分析视角：总分总览 + 本场已录科目（v1.2.4），默认当前学科
    with SessionLocal() as session:
        view_subjects = es.exam_subjects(session, exam_id)
    view_options = ["总分总览"] + view_subjects
    cur_subject = app_config.get_current_subject()
    default_idx = (view_options.index(cur_subject)
                   if cur_subject in view_options else 0)
    view = c3.selectbox("分析视角", view_options, index=default_idx,
                        key="analysis_view")
...
def tab_trends():
    ...
    subject_default = (subject_options.index(cur_subject)
                       if cur_subject in subject_options else 0)
