...
(function () {
  ...
  window.parent.document.addEventListener('keydown', function (e) {
    if (!e.ctrlKey) return;
    var key = e.key;
    if (key === '1') { e.preventDefault(); clickNav('备课'); }
    else if (key === '2') { e.preventDefault(); clickNav('作业'); }
    else if (key === '3') { e.preventDefault(); clickNav('学情'); }
    else if (key === '4') { e.preventDefault(); clickNav('设置'); }
