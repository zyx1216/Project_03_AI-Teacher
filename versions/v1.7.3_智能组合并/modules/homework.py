...
1. 作业管理：新建作业/试卷（弹窗）、混合组卷（题库勾选/AI 即时出题/外部导入/手动加题）、
   调序与分值、智能组卷（所有作业类型）、学生卷/教师卷预览导出、存模板、删除；
...
import config
from models.models import Question, Textbook
from utils.db import SessionLocal
from utils import llm_client, feature_subjects as fs
from utils.app_config import SUBJECT_NAMES, DEFAULT_SUBJECT, DISPLAY_GRADE_CHOICES
...
def _smart_compose_panel(session, hw):
    """智能组卷：章节和知识点约束范围，题库抽题不足时由 AI 补缺。"""
    with st.expander("🤖 智能组卷", expanded=True):
        st.caption("按资料章节、知识点、题型题量和难度配比组卷；题库优先，不足再 AI 补题。")
        subject = hw.subject or DEFAULT_SUBJECT
        ...
        chapters = st.multiselect(
            "选择章节（可选，用于约束 AI 补题）", chapter_options,
            disabled=selected_book is None,
            key=f"smart_chapters_{hw.id}")

        known_kps = qs.get_all_knowledge_points(session, subject)
        picked_kps = st.multiselect(
            "选择知识点（可选，题库和 AI 都按此约束）", known_kps,
            key=f"smart_kps_{hw.id}")
        manual_kps = st.text_input(
            "手动补充知识点（逗号分隔）", key=f"smart_manual_kps_{hw.id}")
        all_kps = list(dict.fromkeys(
            list(picked_kps) +
            [item.strip() for item in re.split(r"[，,、;；]", manual_kps) if item.strip()]
        ))
        ...
        edited_types = st.data_editor(
            default_types, num_rows="dynamic", key=f"smart_type_rows_{hw.id}",
            column_config={
                "题型": st.column_config.TextColumn("题型"),
                "数量": st.column_config.NumberColumn("数量", min_value=1, step=1),
            })
        p1, p2, p3 = st.columns(3)
        ratio_1 = p1.slider("基础占比%", 0, 100, 30, key=f"smart_p1_{hw.id}")
        ratio_2 = p2.slider("中等占比%", 0, 100, 50, key=f"smart_p2_{hw.id}")
        ratio_3 = p3.slider("拓展占比%", 0, 100, 20, key=f"smart_p3_{hw.id}")

        if st.button("🤖 开始智能组卷", type="primary", key=f"smart_run_{hw.id}"):
            if ratio_1 + ratio_2 + ratio_3 != 100:
                st.error("三种难度占比之和必须等于 100%。")
                return
            type_counts = {}
            for row in edited_types.to_dict("records"):
                type_name = str(row.get("题型") or "").strip()
                try:
                    count = int(row.get("数量") or 0)
                except (TypeError, ValueError):
                    st.error("题目数量必须是整数。")
                    return
                if not type_name and count:
                    st.error("题型不能为空。")
                    return
                if not type_name:
                    continue
                storage_type = _smart_storage_type(type_name)
                if storage_type is None:
                    st.error(f"暂不支持的题型：{type_name}")
                    return
                if count <= 0:
                    st.error("题目数量必须大于 0。")
                    return
                type_counts[storage_type] = type_counts.get(storage_type, 0) + count
            if not type_counts:
            ...
            spec = {
                "counts": type_counts,
                "difficulty_ratio": {1: ratio_1, 2: ratio_2, 3: ratio_3},
            }
            ai_context = {
            ...
            with st.spinner("正在智能组卷……"):
                try:
                    result = hw_svc.auto_compose(
                        session, hw.id, spec, all_kps, ai_context=ai_context)
                    session.commit()
                except Exception as exc:
                    st.error(f"智能组卷失败：{exc}")
                    return
            st.success(
                f"题库抽题 {result['rule_picked']} 道，AI 补题 {result['ai_generated']} 道，"
...
def tab_wrong_book():
    ...
    if ec2.button("📄 导出错题本 PDF", key="wb_export_pdf"):
        ...
        st.download_button("⬇️ 下载错题本 PDF", data,
                           file_name=f"{current_subject}错题本.pdf",
                           mime=PDF_MIME, key="wb_export_pdf_dl")
